"""
Evaluate models on PubMedQA via vLLM Chat Completions API with multi-turn search.

Usage:
  python eval_pubmedqa.py --url http://localhost:8002 --model base --label "Base" -n 50
  python eval_pubmedqa.py --url http://localhost:8003 --model ppo200 --label "PPO-200" -n 50
"""
...
def batch_search(query, search_url):
...
def run_sample(api_url, model, chat_prompt, search_url):
    ...
    for turn in range(MAX_TURNS):
        ...
        if action == "answer":
            ...
            search_results = batch_search(query, search_url)
            info_text = (
...
def check_match(predicted, golden):
    ...
    for g in golden:
        if isinstance(g, str):
            gl = g.lower().strip()
            if gl == pred_lower or gl in pred_lower or pred_lower in gl:
                return True
    return False
...
def main():
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="data/pubmedqa_search/test.parquet",
    )
    parser.add_argument(
        "--search_url",
        type=str,
        default="http://127.0.0.1:8001/retrieve",
    )
    ...
    for i, row in df.iterrows():
        chat_prompt = row["prompt"]
        if hasattr(chat_prompt, "tolist"):
            chat_prompt = chat_prompt.tolist()
        golden = row["reward_model"]["ground_truth"]["target"]
        if hasattr(golden, "tolist"):
            golden = golden.tolist()
        question = row["question"]
        test_samples.append((i, chat_prompt, question, golden))

    print("=" * 80)
    print(f"Model:       {label}")
    print(f"API:         {args.url} / {args.model}")
    print(f"Data:        {args.data_path} ({n} samples)")
    ...
    def process_sample(idx):
        sample_idx, chat_prompt, question, golden = test_samples[idx]
        t_start = time.time()
        full_output, turns = run_sample(
            args.url, args.model, chat_prompt, args.search_url
        )
        ...
        result = {
            "index": sample_idx, "question": question, "golden": golden,
        ...
        with print_lock:
            print(
                f"[{cnt:3d}/{n}] #{sample_idx:<4d} | "
                f"turns={len(turns)} searches={n_searches} | "
                f"think={'Y' if has_think else 'N'} answer={'Y' if has_answer else 'N'} | "
                f"{status} gold={golden[0]} pred=\"{(predicted_answer or '')[:30]}\" | {elapsed:.1f}s"
            )

    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        futures = [executor.submit(process_sample, i) for i in range(n)]
    ...
    print("=" * 80)
    print(f"Samples:                    {n}")
    print(f"Total time:                 {total_time:.0f}s ({total_time/n:.1f}s/sample)")
    print(f"Avg turns/sample:           {stats['total_turns']/n:.1f}")
    print(f"Avg searches/sample:        {stats['search_count']/n:.1f}")
    print(f"---")
    print(f"<think>...</think>:         {stats['has_think']}/{n} ({100*stats['has_think']/n:.0f}%)")
    print(f"<search>...</search>:       {stats['has_search']}/{n} ({100*stats['has_search']/n:.0f}%)")
    print(f"<answer>...</answer>:       {stats['has_answer']}/{n} ({100*stats['has_answer']/n:.0f}%)")
    print(f"<information>:              {stats['has_info']}/{n} ({100*stats['has_info']/n:.0f}%)")
    print(f"---")
    print(f"Answer correct (EM):        {stats['correct']}/{n} ({100*stats['correct']/n:.0f}%)")
