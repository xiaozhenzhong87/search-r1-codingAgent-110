...
export EXPERIMENT_NAME=nq-search-r1-grpo-qwen2.5-7b-it-em-paper-fixedkl0.05

# ──────────────────────────────────────────────────────────────────
# Resume: set to the experiment checkpoint directory.
# The code will auto-detect the latest saved step (e.g. global_step_100)
# and continue training from there, appending metrics to the same wandb chart.
# ──────────────────────────────────────────────────────────────────
export RESUME_FROM_CHECKPOINT=verl_checkpoints/$EXPERIMENT_NAME
...
PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo_format \
    ...
    2>&1 | tee logs/${EXPERIMENT_NAME}_resume.log
